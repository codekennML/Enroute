...
import { scheduleFiltersSchema } from '@/app/rider/(verification)/schemas'
    ...
    return (
        ...
        <View className=''>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View>
                    <Text variant="subhead" className='py-3 font-semibold my-3'>Last bus stop</Text>

                    <Controller control={control} name="lastStop" render={({ field: { onChange, value } }) => (
                        <View>
                            <Input
                                // ref={destinationNameRef}
                                placeholder={value ? value : watchedValues?.type === "Travel" ? "Search a  state" : "Search a city"}
                                onFocus={() => setFocusedInput("lastStop")}
                                value={value} onChangeText={onChange}
                                className={`${focusedInput === "lastStop" ? "border-blue-600" : ""} bg-accent h-14 py-3 placeholder:text-foreground px-4 placeholder:text-sm placeholder:font-medium text-xs `} />
                        </View>
                    )} />
                </View>
