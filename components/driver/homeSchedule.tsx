...
const RideItem: React.FC<{ ride: typeof rides[0] }> = ({ ride }) => (
    <Button className='w-full py-2  rounded-md' variant={"ghost"} onPress={() => router.push({
        pathname: "/live/online"
    })} >
        <View className='flex flex-row items-center justify-start gap-x-3 '>
            ...
            <View className='flex-1'>
                <View className='flex-row justify-between items-center '>
                    <Text variant="body" color="primary" className=' tracking-wide text-ellipsis overflow-hidden font-medium '>
                        {`${ride.destination} ${ride.city}`}
                    </Text>
                    <Text variant={"subhead"} className='font-semibold'>{ride.time}</Text>
                </View>
                <View className=''>
